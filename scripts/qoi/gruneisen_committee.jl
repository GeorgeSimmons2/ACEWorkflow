...
for (k, θc) in centres
    ...
    @printf("  %-20s γ(%.0f K) = %+.4f   a = %.5f Å, min ω %+.3f THz, %d/%d modes dropped\n",
            basename(k), T_K, r.γT, r.a0, r.minω, r.n_drop, r.n_modes)
end
...
for e in ensembles
    @printf("%-14s central model (%s): γ = %+.4f\n",
            e.tag, basename(e.centre), centre_res[e.centre].γT)
end
println()
