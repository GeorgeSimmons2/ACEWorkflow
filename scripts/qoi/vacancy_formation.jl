...
# Each ensemble names its OWN central model, because they are not always the same.  For
# Al_12 the unconstrained committee is drawn around lin_params and the constrained one
# around theta_mean; for Al_16 and Al_20 BOTH come from one hypercube around lin_params
# and no theta_mean exists.  ENS*_CENTRE is the literal "lin_params" or a path to a CSV.
...
function load_centre(c)
    ...
    v = vec(readdlm(c, ','))
    length(v) == n_params || error("$c holds $(length(v)) coefficients, model has $n_params")
    return v
end
...
centre_res = Dict{String,Any}(); design = Dict{String,Vector{Float64}}()
for (k, θc) in centres
    ...
    @printf("  %-20s E_f = %+.4f eV   unrelaxed %+.4f eV   relaxation %+.4f eV\n",
            basename(k), r.Ef, r.Ef_unrelaxed, r.Ef - r.Ef_unrelaxed)
...
# the first ensemble's central model, whatever it is called — NOT a hardcoded key,
# since the centres are now names supplied by the caller
mean_res = centre_res[ensembles[1].centre]
...
for e in ensembles
    @printf("%-14s central model (%s): E_f = %+.4f eV\n",
            e.tag, basename(e.centre), centre_res[e.centre].Ef)
end
println()
