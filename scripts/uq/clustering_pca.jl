...
R = kmeans(result.P * pops_corrections', k; maxiter=10_000, display=:iter)
...
cluster_eigvecs, cluster_bounds = multi_hypercube(pops_corrections, R, result.P)

samples = []

for i = 1:length(R.counts)
    eigvecs_i, eigbounds_i = cluster_eigvecs[i], cluster_bounds[i]
    samples_i, _ = sample_hypercube(eigvecs_i, eigbounds_i, result.lin_params; number_of_committee_members=3)
    append!(samples, [vec(samples_i[:,j]) for j=1:size(samples_i, 2)])
end

writedlm("$(result.dir)/$(k)_pca_multi_hypercube_samples.csv", samples, ',')