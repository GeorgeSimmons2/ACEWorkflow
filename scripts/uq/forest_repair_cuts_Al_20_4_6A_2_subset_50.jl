# forest_repair_cuts_Al_20_4_6A_2_subset_50.jl
#
# Guaranteed phonon-stable POPS committee via CONSTRAINT GENERATION (cutting
# planes).  The row-based repair (fixed mean-model eigenmode rows) failed
# because each forest member's true soft mode is not spanned by the mean's
# eigenmodes.  Here detection and cuts use each member's OWN Hessian:
#
#   for member i (θ = lin_params + δθ_i):
#     loop:
#       H = mass-weighted supercell Hessian at θ (folded q-mesh spectrum)
#       soft = eigenvectors with ω² < −tol                (the actual imaginary modes)
#       if none → member is phonon-stable, STOP
#       for each soft eigenvector v: add the linear curvature row
#           row·θ' = (v'D(θ')v)/‖v‖²  ≥  margin           (built by FD of the
#           basis energy along the displacement v/√m — exact, q=0 commensurate)
#       re-solve the constrained-POPS QP (pin Ap[i]·θ̃ = Yw[i], b′=0,
#           Born + b″ + all accumulated cuts) → new θ
#
# Each cut forbids the current worst instability, so the loop converges to a
# member that is stable on the whole commensurate mesh.  Because every member
# still pins its own observation, the committee stays dispersed — physicality
# without collapsing coverage.  Stable forest members (the majority) pass at
# iteration 0 untouched.  Verified with full phonon bands + a test-set
# parity/coverage study.
#
# Outputs → models/Al_20_4_6A_2_subset_50_percent/results/forest_repair_cuts/
#
# Run:  julia --project [-t N] scripts/uq/forest_repair_cuts_Al_20_4_6A_2_subset_50.jl
...
N_cell_detect  = 3      # supercell for the folded-mesh stability check / cuts
N_cell_bands   = 3      # supercell for full-band verification
cut_margin_THz = 0.1    # cut any mode below this (imaginary or near-zero soft)
max_cuts       = 20     # safety cap on cutting-plane iterations per member
test_stride    = 10
...
λ  = 1.0 / size(Ap, 1)
outdir = "$(result.dir)/results/forest_repair_cuts"; mkpath(outdir)
...
# ── Constrained-POPS QP with augmentable constraint set ──────────────────────
Hqp = sparse(Ap' * Ap .+ λ .* (P' * P)); qqp = -(Ap' * Yw)
osqp = OSQP.Model()
function repair(oi, rows, lowers)
    phys = sparse(vcat(b_prime', rows) / P)
    A_full = vcat(sparse(Ap[oi, :]'), phys)
    l_full = vcat([Yw[oi]], [0.0], lowers)
    u_full = vcat([Yw[oi]], [0.0], fill(Inf, length(lowers)))
...
for (k, i) in enumerate(selected)
    ...
    for it in 0:max_cuts
        ...
        if isempty(soft)
            it > 0 && @printf("  obs %6d (%s): stable after %d cut(s), min ω=%+.3f THz\n",
                              i, groups[k], it, fmin)
            break
        end
        ...
        θ, st = repair(i, rows, lowers)
        st == :Solved || @warn "obs $i repair QP status=$st at cut $it"
    end
    committee[k] = θ
end
...
x_vals, all_freqs, x_ticks, labels =
    phonon_committee(model, committee, result, element; N_cell=N_cell_bands, file_prefix="forest_repair_cuts/")
...
ax  = Axis(fig[1, 1]; xlabel="Wave vector", ylabel="Frequency (THz)",
           title="$(result.name) — forest committee, cutting-plane repair",
