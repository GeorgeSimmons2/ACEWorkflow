...
N_cell_fc        = 4           # 3×3×3 for the undotted band-path Hessian
N_per_seg        = 20
n_lev, n_res, n_rand = 5, 10, 15
committee_subdir = "bandpath_undotted_ncell4_densek"   # where committee_{repaired,rejection}.csv live
...
outdir = "$(result.dir)/results/npt_thermal_expansion_naive_worst_member"; mkpath(outdir)
...
rep = readdlm("$committee_dir/committee_rejection.csv", ',')    # 30 × n_params
...
θ_npt = vec(readdlm("/storage/astro2/phupfb/PhD/acestuff/ACEWorkflow/models/Al_12_4_6A_2_/results/npt_thermal_expansion_constrained_worst_member/theta_naive_worst.csv", ','))
