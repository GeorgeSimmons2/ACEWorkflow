...
bounds = (Vector{Float64}(zbl_energies), Vector{Float64}(ones(length(zbl_energies)) .* Inf))
...
constrained_parameters = constrained_ridge_regression(Ap, Yw, Gamma, constraint_matrix, bounds)

# ACEpotentials.Models.set_linear_parameters!(model, constrained_parameters)
...
constrained_bulk_energies = [dot(bulk_basis, constrained_parameters) for bulk_basis in full_bulk_bases]
...
lines!(ax, full_lattice_constants, constrained_bulk_energies;
       color=:steelblue, linewidth=2, label="ACE with constrained core")
lines!(ax, full_lattice_constants, unconstrained_bulk_energies;
       color=:orange, linewidth=2, linestyle=:dash, label="ACE with unconstrained core")
...
save("$(result.dir)/results/no_clamp_constraining_core_repulsion.png", fig)