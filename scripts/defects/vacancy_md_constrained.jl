...
sys = deleteat!(bulk(element, a=a_eq*u"Å", cubic=true) * (3,3,3), 1)
sys_md = Molly.System(sys; force_units=u"eV/Å", energy_units=u"eV")
temp = 500.0 * u"K"
dt =  1.0u"fs"
n_steps   = 100_000
log_every = 50
dir_name  = "$(result.dir)/results/constrained_$(ustrip(dt) * n_steps)_fs_NVT"
mkpath(dir_name)
sys_md = Molly.System(sys_md;
                      ...
                      loggers=(temp   = Molly.TemperatureLogger(log_every),
                               ...
                               writer = Molly.TrajectoryWriter(20*log_every, "$(dir_name)/traj_log.xyz"),
                               energy = Molly.PotentialEnergyLogger(typeof(1.0u"eV"), log_every),))

simulator = Molly.VelocityVerlet(
   dt = dt,
   coupling = AndersenThermostat(temp, 500*dt))
