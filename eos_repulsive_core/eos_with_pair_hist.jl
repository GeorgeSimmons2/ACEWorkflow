# ─────────────────────────────────────────────────────────────────────────────
# COPY of scripts/repulsive_core/ZBL_core_ACE_correction.jl — the script that produced
# models/W_20_4_5A_3/results/eos_with_pair_hist.pdf.
#
# Differences from the original, all marked `# [REPRO]`:
#   * `using Test` and `using DelimitedFiles` hoisted to the top.  The original has an
#     embedded @testset but never imports Test, and calls readdlm ~30 lines BEFORE its
#     own `using DelimitedFiles`.  Run top to bottom it therefore dies twice over — it
#     only ever worked in a REPL that already had both loaded.  Both confirmed by
#     running it.
#   * the training set is addressed from the repo root instead of the process working
#     directory, so it no longer only works when launched from one particular place
#   * the unused POPS block is guarded off — it is dead code that also crashes
#   * the output stem is overridable, so a rerun need not overwrite the published figure
# No physics, fit or plotting is changed.
#
# Run:  julia --project -t 8 eos_repulsive_core/eos_with_pair_hist.jl
# ─────────────────────────────────────────────────────────────────────────────
using ACEpotentials, EmpiricalPotentials, ACEWorkflow, OSQP, SparseArrays
using Test            # [REPRO] the embedded @testset needs this; the original never imports it
using DelimitedFiles  # [REPRO] readdlm is used ~30 lines before the original's `using DelimitedFiles`
using Printf          # [REPRO] for the leverage diagnostic below
const REPO = normpath(joinpath(@__DIR__, ".."))   # [REPRO] anchor paths to the repo root
...
# [REPRO] This POPS block is DEAD CODE and it also CRASHES.
# Dead: pops_eig, pops_bound and con_pops_samples are assigned here and never referenced
# again — nothing below this line uses them, and the figure does not depend on them.
# Crashes: hypercube() throws `ArgumentError: matrix contains Infs or NaNs`, because
# constrained_pointwise_corrections divides by `leverage`, which contains entries at or
# near zero (printed above).  Guarded off by default so the script runs; set POPS_BLOCK=1
# to re-enable it and reproduce the failure.
@printf("leverage: min %.3e, max %.3e, %d entries < 1e-12 of the max\n",
        minimum(leverage), maximum(leverage),
        count(<(1e-12 * maximum(leverage)), leverage))
if get(ENV, "POPS_BLOCK", "0") != "0"
...
# [REPRO] root-anchored, and overridable, so this does not depend on the launch directory
TRAINSET = get(ENV, "TRAINSET", joinpath(REPO, "data", "W", "df_W_train.extxyz"))
isfile(TRAINSET) || error("training set not found at $TRAINSET — set TRAINSET")
atoms = ExtXYZ.load(TRAINSET)
...
fig = Figure(
    size = (900, 667),   # taller for stacked panels
...
vlines!(ax2, [lattice_constants[end]];
    color=(:black, 0.7), linewidth=3,
    linestyle=:dash,
    label="Maximum"
)
...
h = normalize(fit(Histogram, all_pairs, edges))
...
# set OUT to write elsewhere and leave the published figure alone
OUT = get(ENV, "OUT", "$(result.dir)/results/eos_with_pair_hist")
save("$OUT.pdf", fig)
save("$OUT.png", fig; px_per_unit = 2)
println("figure → $OUT.{pdf,png}")