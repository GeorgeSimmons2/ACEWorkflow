# ─────────────────────────────────────────────────────────────────────────────
# PINNED REPRODUCTION COPY of scripts/uq/bandpath_committee_undotted_Al_12_4_6A_2_multivolume.jl
#
# Stage 1 ("constrain") of the pipeline behind thermal_expansion_vs_experiment/.
# Differences from the original are marked `# [REPRO]` and listed in README.md:
# a pinned OSQP rho schedule, and an output directory that defaults somewhere fresh so
# a rerun cannot overwrite the committee the published figure depends on.
# No constraint, sampler or physics setting is changed.
# ─────────────────────────────────────────────────────────────────────────────
...
# ── [REPRO] DETERMINISM ──────────────────────────────────────────────────────
# Rerunning the ORIGINAL of this script under an identical seed does not reproduce its
# members.  Measured, not assumed: results/bandpath_undotted_multivolume_rerun/ vs
# results/bandpath_undotted_multivolume/ differ by max |Δθ| = 4.31 on the softest
# member, while the naive arm — same sampler, same seed, NO QP — is bit-exact.  So the
# non-determinism is in the QP, not the RNG.
#
# The only timing-dependent input in the QP path is OSQP's rho schedule.  With
# `adaptive_rho_interval = 0` (the default, and what the original leaves it at) OSQP
# derives the number of iterations between rho updates from MEASURED SETUP AND ITERATION
# TIME.  Verified: on a probe QP the effective value is filled in automatically from 0
# to 50.  Different wall-clock timing therefore gives a different rho schedule, a
# different iterate path, and — after a cutting-plane cascade of ~864 rows per member —
# a completely different accepted vector.
#
# RHO_INTERVAL pins that schedule to a fixed iteration count, removing the dependence.
# NOT YET PROVEN TO BE SUFFICIENT: on the small probe QP the timing-derived interval
# happened to be stable across processes, so the mechanism is demonstrated but the cure
# is not.  Verify it on the real problem before relying on it:
#     bash npt_trajectories/run_pipeline.sh verify-determinism
# which runs this stage twice into separate directories and reports max |Δθ|.
#
# Pinning changes the rho schedule, so this will NOT reproduce the previously published
# members — it makes future runs reproducible, it does not recover the past ones.
...
# [REPRO] fresh by default; the published committee is bandpath_undotted_multivolume
outdir = get(ENV, "COMMITTEE_OUT", "$(result.dir)/results/repro_bandpath_undotted_multivolume"); mkpath(outdir)
...
function constrain_member(i, extra_rows, extra_lower)
    ...
    OSQP.setup!(osqp; P=Hqp, q=qqp, A=A_full, l=l, u=u, max_iter=4_000_000, check_termination=25, verbose=false, eps_abs=1e-6, eps_rel=1e-6, adaptive_rho_interval=RHO_INTERVAL)   # [REPRO] see the determinism note
