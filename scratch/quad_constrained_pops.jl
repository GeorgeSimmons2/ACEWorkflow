...
function iterative_constrained_pops(X, Y, Gamma, A_constraints, l_constraints, u_constraints; H=nothing)
    ...
    for i in 1:n_points
        ...
        push!(constrained_corrections, Gamma \ delta_theta)
    end
    
    # Return corrections as matrix (each row = one member's corrections)
    return hcat(constrained_corrections...)'
end
...
constrained_corrections = iterative_constrained_pops(X, y, Gamma, 
                                                                A_constraints, l_constraints, u_constraints)
pointwise_corrections_constrained = constrained_corrections .+ nominal_coeffs'
...
ax2 = Axis(fig[1, 2], xlabel="x", ylabel="y", 
          title="Constrained POPS: Quadratic with STRICT bounds\\n(respects bounds everywhere!)")
...
ax_err = Axis(fig2[1, 1], xlabel="x", ylabel="Absolute Error", 
